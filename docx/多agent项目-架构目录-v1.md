...
- **阶段一（现在）**：本地验证基础论证——docker compose 本地跑通全链路
...
| 6 | **沙箱** | ✅ **OpenSandbox**（阿里开源，github.com/alibaba/OpenSandbox，Apache 2.0） | sandbox/ 层改为 OpenSandbox 适配（adapter + opensandbox 实现，多语言 SDK + 统一 API）；**内置 Docker/K8s 运行时 → 本地可自托管，非纯托管云服务**；含 MCP Server 集成（Agent 项目天然契合）；⚠️ 本地部署细节以官方 README 为准 |
...
| # | 待决策 | 选项 | 我的建议 | 影响 |
|---|--------|------|----------|------|
| **3** | OpenSandbox 本地开发模式 | ✅ 基本关闭：OpenSandbox 是**开源可自托管**平台（内置 Docker 运行时）→ 本地直接部署 OpenSandbox 跑开发，生产用 K8s 运行时/云托管，**同一套 SDK，无替身** | 无需二选一；本地部署细节开工时按官方 README 落地 | 沙箱层实现 |
...
- [x] **Python 3.11+** — 已有
- [x] **Node.js + pnpm** — 已有（当前项目在用）
- [x] **Docker Desktop** — 已有（本地）
- [x] **DeepSeek API Key** — 已有（当前项目 .env）
- [x] **豆包 / 火山方舟 Key** — 已有（config.py 的 ark_）
- [ ] **智谱 Key**（可选，多 provider 备选）— open.bigmodel.cn 注册
- [ ] **LangSmith API Key**（监控，唯一必缺）— langsmith.com 注册，免费额度，约 5 分钟
- [ ] **OpenSandbox 本地部署**（Docker 运行时自托管，按官方 README）— 开工时我协助落地
- [ ] **Git 仓库初始化**（GitHub 私有仓库，部署 runner 需要）

### 7.2 部署阶段（本地验证通过后再做）

- [ ] **云服务器**：2核4g（阿里云 ECS / 腾讯云轻量），准备账号 + 密码/密钥 + 公网 IP
- [ ] **域名**（可选，演示公网访问好看；不买可用 IP:端口）
- [ ] **生产 .env 副本**：模型 API Key 在服务器上再填一份（密钥不入 git）
- [ ] **LangSmith Key 生产配置**
- [ ] **OpenSandbox 生产形态**（K8s 运行时 / 云托管，用到时再看）
- [ ] **阿里云实名认证**（云资源前提）
