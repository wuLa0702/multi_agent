...
> **关联文档**：
> - 本地方案家族：`docs/decisions/2026-08-06-设计-记忆能力开发-v3.md`（本地文件 + Store 记忆——降级方案的兜底实现）
> - 架构真相源：`docs/架构/2026-08-02-多agent项目-架构目录-v1.md`（依赖单向：api → agent → memory）
> - 配置中心：`backend/src/core/config.py`（BaseSettings + .env 切换）
> - 文档规范：`docs/2026-08-02-文档规范.md`
