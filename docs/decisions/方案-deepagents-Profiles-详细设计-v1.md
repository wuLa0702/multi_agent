# deepagents Profiles 详细设计 v1（官方文档梳理 → 项目落地 · 评审稿）
...
1. 把官方 Profiles 文档翻译成"字段 × 语义 × 注意点"的完整梳理，不再每次查原文
2. 设计本项目如何用（注册模块 / key 策略 / 字段映射），并说清**为什么不用**的部分
3. 核心代码可直接运行（含测试），评审通过后即可落地
...
| 在本设计内 | 不在本设计内 |
|-----------|-------------|
| HarnessProfile 完整梳理 + 落地设计 | ProviderProfile 落地（§4.3 论证不适用） |
| 模型级 key 策略与注册模块 | YAML 配置管理（HarnessProfileConfig，§5.5 论证不做） |
| 与既有运行时模型切换的分工边界 | 插件 entry point 分发（单项目无此需求） |
| 核心代码 + 测试 + 接线 | per-request 动态 profile（官方无此能力，P2 观望） |
...
### 2.1 两类 Profile <span style="color:#fff;background-color:#e11d48;border-radius:4px;padding:1px 6px;font-size:0.8em">v1</span>
...
### 2.2 HarnessProfile 字段表（7 字段） <span style="color:#fff;background-color:#e11d48;border-radius:4px;padding:1px 6px;font-size:0.8em">v1</span>
...
### 2.3 注册 key 与解析 <span style="color:#fff;background-color:#e11d48;border-radius:4px;padding:1px 6px;font-size:0.8em">v1</span>
...
### 2.4 合并语义表 <span style="color:#fff;background-color:#e11d48;border-radius:4px;padding:1px 6px;font-size:0.8em">v1</span>
...
### 2.5 配置文件与插件 <span style="color:#fff;background-color:#e11d48;border-radius:4px;padding:1px 6px;font-size:0.8em">v1</span>
...
### 2.6 内置 profile（0.7.1 实测） <span style="color:#fff;background-color:#e11d48;border-radius:4px;padding:1px 6px;font-size:0.8em">v1</span>
...
### 3.1 现有模型选择机制 <span style="color:#fff;background-color:#e11d48;border-radius:4px;padding:1px 6px;font-size:0.8em">v1</span>
...
### 3.2 现有组装点 <span style="color:#fff;background-color:#e11d48;border-radius:4px;padding:1px 6px;font-size:0.8em">v1</span>
...
### 3.3 接线点 <span style="color:#fff;background-color:#e11d48;border-radius:4px;padding:1px 6px;font-size:0.8em">v1</span>
...
### 4.1 ⚠️ ChatOpenAI 实例一律推导 provider="openai" <span style="color:#fff;background-color:#e11d48;border-radius:4px;padding:1px 6px;font-size:0.8em">v1</span>
...
### 4.2 ⚠️ Profile 是构建期解析，运行期换模型不重解析 <span style="color:#fff;background-color:#e11d48;border-radius:4px;padding:1px 6px;font-size:0.8em">v1</span>
...
### 4.3 ProviderProfile 不适用于本项目 <span style="color:#fff;background-color:#e11d48;border-radius:4px;padding:1px 6px;font-size:0.8em">v1</span>
...
### 5.1 决策总览 <span style="color:#fff;background-color:#e11d48;border-radius:4px;padding:1px 6px;font-size:0.8em">v1</span>
...
### 5.2 key 策略 <span style="color:#fff;background-color:#e11d48;border-radius:4px;padding:1px 6px;font-size:0.8em">v1</span>
...
### 5.3 注册模块与时机 <span style="color:#fff;background-color:#e11d48;border-radius:4px;padding:1px 6px;font-size:0.8em">v1</span>
...
### 5.4 HarnessProfile 字段落地映射 <span style="color:#fff;background-color:#e11d48;border-radius:4px;padding:1px 6px;font-size:0.8em">v1</span>
...
### 5.5 不做清单 <span style="color:#fff;background-color:#e11d48;border-radius:4px;padding:1px 6px;font-size:0.8em">v1</span>
...
## 6. 分：边界与取舍 <span style="color:#fff;background-color:#e11d48;border-radius:4px;padding:1px 6px;font-size:0.8em">v1</span>
...
### 7.1 `backend/src/agent/profiles.py`（全量，可直接运行） <span style="color:#fff;background-color:#e11d48;border-radius:4px;padding:1px 6px;font-size:0.8em">v1</span>
...
### 7.2 `backend/src/api/main.py` 接线（lifespan 内） <span style="color:#fff;background-color:#e11d48;border-radius:4px;padding:1px 6px;font-size:0.8em">v1</span>
...
### 7.3 `backend/tests/test_agent/test_profiles.py`（全量） <span style="color:#fff;background-color:#e11d48;border-radius:4px;padding:1px 6px;font-size:0.8em">v1</span>
...
## 8. 分：测试设计 <span style="color:#fff;background-color:#e11d48;border-radius:4px;padding:1px 6px;font-size:0.8em">v1</span>
...
## 9. 总：验收清单与风险自检 <span style="color:#fff;background-color:#e11d48;border-radius:4px;padding:1px 6px;font-size:0.8em">v1</span>
