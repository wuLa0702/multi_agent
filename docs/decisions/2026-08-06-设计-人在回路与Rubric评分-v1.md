...
### 5.1 D1：HITL 审批分级配置（interrupt_on + 权限 interrupt 双轨） ⚪ v1
...
### 5.2 D2：审批决策 API（复用 /v1/chat/stream） ⚪ v1
...
### 6.1 V1（必做）：RubricMiddleware 在国产模型的可行性 ⚪ v1
...
### 6.2 V2（必做）：astream_events 下 interrupt 检测与 Command(resume) 恢复形态 ⚪ v1
...
### 7.1 `backend/src/agent/hitl.py`（HITL 配置，全量） ⚪ v1
...
### 7.2 main_agent 接线（HITL + Rubric 增量） 🔴 v1.2
...
### 7.3 chat.py 审批 API 增量（设计 §5.2/§5.3） ⚪ v1
