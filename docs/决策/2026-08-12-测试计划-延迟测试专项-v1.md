...
| 3 | 前端单测 | ✅ 49 全绿 | chatStore/sse 48 + 多 action N 卡用例（前端 HITL 契约单测层通过）|
| 3 | **项 6/8** 浏览器 + E2E | ⏳ **待环境** | 前端**无 Playwright/E2E 配置**（仅 vitest）；项 6 浏览器四场景 + 项 8 E2E 需装 Playwright + 真实浏览器 + 前后端联调——**非代码阻塞**，环境补齐后测 |
| 4 | **单价预填**（co-creator 指示）| ✅ 完成 | 搜真实官方价并落库：deepseek-v4-flash 0.001/0.002、pro 0.003/0.006、doubao-seed 0.006/0.030、2.0-Code 0.0032/0.016、glm-4-plus 0.05/0.05（元/千 token）。<br/>**验证**：真实对话成本核算 > 0（571 tokens → 0.000571 元）✓；顺带验证 R 容错真实链路（搜索 403/429 重试耗尽降级不中断）。seed 单价结构也改为 dict 预填。 |
