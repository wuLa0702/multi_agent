...
> 设计决策与拍板记录：见 `docs/多agent项目-架构目录-v1.md`（v2.3，权威蓝图）。
...
## 文档规范（docs/ 下所有文档强制）
- **中文文件名**（如 `docs/方案-后端基础架构-v1.md`），附件图片/OCR 除外
- 每个文档顶部必须含规范头：📋 规范引用 + 📌 更新时间 + 🔄 最近三次改动 + 目录
- 格式规则唯一载体：`docs/文档规范.md`（改规则只改它，不在各文档里重复抄规则）
- 每次更新维护固定动作：更新时间 → 最近三次改动表 → 版本表 → 文件名版本号（大改升级）
- 详见 `docs/文档规范.md`
